...
_TOOLS = [
    (TOOL_SELECT, "↖ 選択",  "クリックで選択・ドラッグで移動/拡大"),
    (TOOL_PEN,    "✏ ペン",   "フリーハンド描画"),
    (TOOL_PATH,   "📐 パス",  "クリックで点追加、ダブルクリックで確定"),
    (TOOL_LINE,   "╱ 直線",   "2点間の直線"),
    (TOOL_RECT,   "□ 矩形",   "ドラッグで矩形"),
    (TOOL_CIRCLE, "○ 楕円",   "ドラッグで楕円"),
]
...
class RawToolbar(QWidget):
    ...
    img_params_toggled  = pyqtSignal(bool)   # True = show GIMP params panel
    ...
    def __init__(self, parent=None):
        ...
        for tid, label, tip in _TOOLS:
            ...
            btn.setToolTip(tip)
            btn.setStyleSheet(
                "QPushButton{font-size:12px;border:1px solid #555;"
                "border-radius:3px;background:#333;padding:0 4px;}"
        ...
        # Image-mode only: toggle between placement view and GIMP params view
        self._img_params_btn = QPushButton("🔬 抽出設定")
        self._img_params_btn.setCheckable(True)
        self._img_params_btn.setFixedHeight(28)
        self._img_params_btn.setToolTip("画像の輪郭抽出パラメーターを表示/非表示")
        self._img_params_btn.setStyleSheet(
            "QPushButton{border:1px solid #555;border-radius:3px;background:#333;padding:0 6px;}"
            "QPushButton:checked{background:#446655;border-color:#88ccaa;}"
            "QPushButton:hover{background:#444;}")
        self._img_params_btn.toggled.connect(self.img_params_toggled)
        self._img_params_btn.setVisible(False)  # hidden until image mode
        hl.addWidget(self._img_params_btn)
    ...
    def set_image_mode(self, is_image: bool):
        """Show/hide the image params toggle button."""
        self._img_params_btn.setVisible(is_image)
        if not is_image:
            self._img_params_btn.setChecked(False)
